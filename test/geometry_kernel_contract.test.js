...
test('geometry kernel circle contract covers hit test, segments, bounds and handles', () => {
  ...
  assert.equal(getObjectGeometryKind(circleObject), 'circle');
  assert.deepEqual(getGeometryDisplayDimensions(circleObject), { width: 50, height: 50, radius: 25 });
...
test('geometry kernel resolves object geometry contract and rejects shape-only legacy fallback', () => {
  ...
  assert.equal(segments.length, 3);
  assert.equal(getObjectGeometryKind(triangleObject), 'polygon');
  assert.deepEqual(getGeometryDisplayDimensions(triangleObject), { width: 60, height: 40, radius: 20 });
  ...
  assert.equal(resolveObjectGeometry(legacyVerticesOnly), null);
});
